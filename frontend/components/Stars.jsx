...
// Koʻrsatish rejimi — kasrli reytingni aniq toʻldiradi (masalan 4.4)
export function StarRating({ value = 0, size = 16, className = '' }) {
  ...
  return (
    <span className={`relative inline-flex align-middle ${className}`} aria-label={`${value} / 5`}>
      ...
      {/* Toʻldirilgan qatlam — kenglik boʻyicha kesiladi */}
      <span
...
// Kichik reyting koʻrsatkichi: yulduzlar + oʻrtacha (soni)
export function RatingBadge({ average = 0, count = 0, size = 14, showCount = true }) {
  if (!count) {
    return <span className="text-xs text-muted">Baho yoʻq</span>;
  }
  return (
